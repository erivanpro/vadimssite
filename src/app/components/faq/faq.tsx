    ...
    return (
      ...
      <div className="max-w-3xl mx-auto mt-10 space-y-6">
        {faqs.map((faq, index) => (
          ...
          >
            <Disclosure as="div" className="faq-card">
              {({ open }) => (
                <>
                  <DisclosureButton className="flex w-full justify-between gap-6 text-left text-lg font-bold text-gray-900">
                    <span>{t(faq.question)}</span>
                    <span>
                      {open ? (
                        <MinusSmallIcon className="h-6 w-6 text-[#52bf88]" />
                      ) : (
                        <PlusSmallIcon className="h-6 w-6 text-[#52bf88]" />
                      )}
                    </span>
                  </DisclosureButton>
                  <DisclosurePanel as="div" className="mt-4 text-base leading-relaxed text-gray-600">
                    {t(faq.answer)}
                  </DisclosurePanel>
                </>
              )}
